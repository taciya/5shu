...
async function submitForm() {
  ...
  const birthData = parseNatalTime(birthTime) || {
    year: null,
    month: null,
    day: null,
    hour: null,
    minute: null,
  } // 解析 natalTime，若无效则使用默认值
  const birthPlace = document.getElementById('birthPlace').value
  // 新增：检查是否有 natalTime
  const natalTime = document.getElementById('natalTime').value.trim()
  const natalData = parseNatalTime(natalTime) || {
    year: null,
    month: null,
    day: null,
    hour: null,
  ...
  const trueSolarNatalTime = calculateTrueSolarTime(
    natalData.year,
    natalData.month,
    natalData.day,
    natalData.hour,
  ...
  try {
    ...
    data.ziwei_chart.shichen = `${String(trueSolarTime.hour).padStart(2, '0')}:${String(trueSolarTime.minute).padStart(2, '0')}`

    data.natal_chart = JSON.parse(data.natal_chart) || {} // 确保 natal_chart 存在
    data.natal_chart.name = formData.name

    data.natal_chart.shichen = `${String(trueSolarTime.hour).padStart(2, '0')}:${String(trueSolarTime.minute).padStart(2, '0')}`
    // 渲染命盘
