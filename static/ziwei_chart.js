...
function initDivination() {
  ...
  divinationBtn.addEventListener('click', function () {
    ...
    const gender =
      document.getElementById('gender').value === 'male' ? '男' : '女'
...
async function queryKnowledgeBase(question) {
  ...
  try {
    const response = await fetch(
      'https://shu-service-437366723887.us-central1.run.app/api/v1/query/kb-001',
      {
        method: 'POST',
        headers: {
...
function renderChart(mixData, formData) {
  ...
  if (data.palaces && Array.isArray(data.palaces)) {
    data.palaces.forEach((palace) => {
      const palaceElement = createPalaceElement(
        palace,
        data.three_level_hexagram,
        natalData.palaces,
      )
      if (palaceElement) {
...
function generateFeigongString() {
  ...
  if (liunianSelector.value) {
    ...
    if (liunianPalace) {
      const name = liunianPalace
        .querySelector('.palace-name')
        .textContent.trim()
  ...
  if (dayunInfo !== '未选') {
    ...
    fullContent += `基本信息：来因宫=${getLaiyinPalace(palaces).isLaiyin.trim()} , 大运=${dayunInfo || '不分析'} , 流年=${liunianStr || '不分析'} , 当前年份=${new Date().getFullYear()}\n`
  } else {
...
function createPalaceElement(palace, three_level_hexagram, natalPalaces) {
  ...
  const NatalPalace = natalPalaces
    ? natalPalaces.find((p) => p.dizhi === palace.dizhi)
    : null
  if (NatalPalace) {
...
function addBrightnessToStar(starElement, starName, palace, starType) {
  ...
  const brightnessMap =
    starType === 'main'
      ? palace.main_stars_brightness
      : palace.minor_stars_brightness
...
function updateMinorStarsPosition(mainStarsContainer) {
  ...
  // 设置CSS变量
  document
    .getElementById(minorStarsID)
    .style.setProperty('--main-stars-width', `${width}px`)
}
...
function drawFlySihuaConnections(sourcePalace, flySihua) {
  ...
  for (const [sihuaType, info] of Object.entries(flySihua)) {
    ...
    if (targetElement) {
      ...
      const taiji = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'text',
      )
      ...
      endDot.setAttribute(
        'class',
        `connection-dot end ${sihuaType.toLowerCase()}`,
      )
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  exportBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      // 验证必填参数
      if (
        !chartParams.birthYear ||
        !chartParams.birthMonth ||
        !chartParams.birthDay
      ) {
...
const bindStarEvents = () => {
  ...
  document.addEventListener('click', (e) => {
    const isStar = e.target.closest(
      '.main-star, .minor-star, .xiaoxing-star, .shensha-star, .changsheng-star',
    )
    if (!isStar) {
      hideStarMeaning()
    }
...
function createTrackingMark(sourceTianGan, sihuaType, starName, dizhi) {
  ...
  targetPalaces.forEach((palaceElement) => {
    ...
    // 创建标记容器（如果不存在）
    let marksContainer = palaceElement.querySelector(
      '.tracking-marks-container',
    )
    if (!marksContainer) {
    ...
    const brightness = STAR_BRIGHTNESS_TABLE[sihuaType]
      ? STAR_BRIGHTNESS_TABLE[sihuaType][dizhi]
      : ''
...
function findPalacesByTianGan(tianGan) {
  ...
  palaceElements.forEach((palace) => {
    ...
    if (
      ganzhiElement &&
      tianGan.includes(ganzhiElement.textContent.substring(0, 1))
    ) {
      palaces.push(palace)
    }
  })
...
async function showPalaceMeaning(palaceName, palaceGan, sihuaData, element) {
  ...
  if (
    meaning.examples &&
    meaning.examples.feichu.length > 0 &&
    meaning.examples.feiru.length > 0
  ) {
    content += `
            <div style="margin-bottom:10px;">
  ...
  if (palaceGan && Object.keys(sihuaData).length > 0) {
    try {
      ...
      const response = await fetch(
        getApiBaseUrl() +
          `/api/sihuas/${encodeURIComponent(palaceGan)}?${queryParams.toString()}`,
        {
          method: 'GET',
...
function findPalaceByStar(starName) {
  ...
  for (let i = 0; i < palaces.length; i++) {
    ...
    if (foundStarEl) {
      ...
      let palaceName = palaceEl
        .querySelector('.palace-footer .palace-name')
        .textContent.trim()
      if (!palaceName.endsWith('宫')) palaceName += '宫'

      // 提取地支 (根据干支 <span> 内的文本，如 "丙寅" 取第二个字 "寅")
      // 也可以通过 id="寅宫" 提取，这里采用解析干支的方式更加严谨
      const ganzhi = palaceEl
        .querySelector('.palace-footer .ganzhi span')
        .textContent.trim()
      const zhi =
        ganzhi.length >= 2
          ? ganzhi.charAt(1)
          : palaceEl.id.replace('宫', '').trim()
...
function hidePalaceMeaning() {
  ...
  if (
    tooltip &&
    currentDisplayedStar &&
    currentDisplayedStar.startsWith('palace_')
  ) {
    // tooltip.style.display = 'none';
    // currentDisplayedStar = null;
  }
...
function bindPalaceNameEvents() {
  ...
  newPalaceNameElements.forEach((element) => {
    ...
    const ganzhiElement =
      element.parentElement && element.parentElement.querySelector('.ganzhi')
        ? element.parentElement.querySelector('.ganzhi')
        : null
    // 点击事件
...
function updatePalaceDayunName(data, dayunData) {
  ...
  const selectedIndex = sortedPalaces.findIndex(
    (p) => p.dizhi === dayunData.dizhi, // 通过地支唯一标识宫位
  )
...
function clearDayunDisplays() {
  ...
  document.querySelectorAll('.palace').forEach((palaceEl) => {
    ...
    if (palaceNameEl) {
      ...
      natalNameEl.textContent = `${displayName.substring(0, 1)} ${naTalAgeRange}`
...
function calculateLiunianPalace(data, selectedYear) {
  ...
  if (!selectedYear || !data)
    return data.palaces.find((p) => p.name === '命宫') || data.palaces[0]
...
function updatePalaceLiunianName(data, liunianData) {
  ...
  const selectedIndex = sortedPalaces.findIndex(
    (p) => p.dizhi === liunianData.dizhi, // 通过地支唯一标识宫位
  )
  if (selectedIndex === -1) return // 未找到选中宫位，终止执行
...
function editFeigongstr_bak(feigongstr) {
  ...
  const updatedBlocks = blocks.map((block) => {
    ...
    // 从Map中获取该宫位的大运/流年信息
    const { dayunName, liunianName } = palaceInfoMap.get(ageRange) || {
      dayunName: '无',
      liunianName: '无',
    }
    ...
    const shengongLineIndex = lines.findIndex((line) =>
      line.startsWith('  身宫标记: '),
    )
