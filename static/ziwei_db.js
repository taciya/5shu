...
class StarMeaningDB {
  ...
  async getMeaning(starName) {
    try {
      ...
      const response = await this.executeApiCall({
        method: 'GET',
        endpoint: `/stars/${encodeURIComponent(starName)}/dizhi`,
      })
...
async function loadMeaning(starName) {
  ...
  if (meaning) {
    ...
    document.getElementById('combinationMeaning').value =
      meaning.combination || ''
    ...
    document.getElementById('relationshipImpact').value =
...
async function showStarMeaning(starName, element) {
  ...
  try {
    ...
    const tooltip =
      document.getElementById('star-tooltip') || createStarTooltip()
...
function convertStarMeaningsAdvanced(starMeaningMap) {
  ...
  Object.entries(starMeaningMap).forEach(([starName, htmlContent]) => {
    ...
    if (sections.length >= 2) {
      ...
      // 组合象义（第二个div）
      const combinationMatch = sections[1].match(
        /<div>([\s\S]*?)<\/div>\s*<div>([\s\S]*?)<\/div>/,
      )
