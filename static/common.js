...
const SOLAR_LONGITUDE = [
  285.0, 300.0, 315.0, 330.0, 345.0, 0.0, 15.0, 30.0, 45.0, 60.0, 75.0, 90.0,
...
class ElementSize {
  ...
  static getFullDimensions(element, options = {}) {
    ...
    if (boxSizing === 'border-box') {
      ...
      contentWidth =
        clientWidth - paddingLeft - paddingRight - borderLeft - borderRight
      contentHeight =
        clientHeight - paddingTop - paddingBottom - borderTop - borderBottom
    } else {
  ...
  static getSize(element, type = 'border') {
    ...
    switch (type.toLowerCase()) {
      ...
      default:
        throw new Error(
          `不支持的尺寸类型: ${type}。支持的类型: content, padding, border, margin, scroll, client, offset`,
        )
    }
  }
