...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // 绑定新分类对话框按钮事件
  document
    .getElementById('confirmNewCategory')
    .addEventListener('click', createNewCategory)
  document
    .getElementById('cancelNewCategory')
    .addEventListener('click', hideNewCategoryDialog)
...
async function loadAllCategories() {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `加载分类失败: ${errorData.message || response.statusText}`,
      )
    }
...
async function loadSavedMingpanList(category = 'all', flg = false) {
  ...
  try {
    ...
    let url =
      getApiBaseUrl() +
      `/get_mingpan?category=${encodeURIComponent(category)}&device_id=${encodeURIComponent(deviceId)}`
    let passwordInfo
...
function updateCategorySelect() {
  ...
  if (
    selectedValue &&
    Array.from(select.options).some((opt) => opt.value === selectedValue)
  ) {
...
async function confirmDelete() {
  ...
  try {
    ...
    const response = await fetch(
      getApiBaseUrl() + `/delete_mingpan/${currentDeletingId}`,
      {
        method: 'DELETE',
      },
    )
...
function createNewCategory() {
  ...
  if (newOptionIndex !== -1) {
    categorySelect.insertBefore(
      newOption,
      categorySelect.options[newOptionIndex],
    )
  } else {
