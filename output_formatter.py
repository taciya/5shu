...
class OutputFormatter:
    ...
    def format_qimen_runtime(self):

        q = self.ziwei_chart.qimen_runtime
        ...
        lines.append(
            f"  是否存在: {kongwang.get('存在', False)}"
        )

        lines.append(
            f"  空亡强度: {kongwang.get('强度', 0)}"
        )
        ...
        lines.append(
            f"  动态等级: {yima.get('动态等级', 0)}"
        )

        lines.append(
            f"  高动态局: {yima.get('高动态', False)}"
        )
